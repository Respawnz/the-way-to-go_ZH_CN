...
如果文件不存在或者程序没有足够的权限打开这个文件，这句代码`inputFile, inputError = os.Open("input.dat")`将会返回错误。如果文件打开正常，我们就使用 `defer inputFile.Close()` 语句确保在程序退出前关闭该文件。然后，我们使用 `bufio.NewReader` 来获得一个读取器变量。
